...
class MasterModel extends \think\Model
{
    ...
    function __construct($table = '')
    {
        ...
        parent::__construct();
        if ($table != '') {
            if (strpos($this->table, $pix) !== FALSE) {
                $this->setTable($this->table);
            } else {
                $this->setTable($this->prefix . $this->table);
            }
        }
    ...
    function getFieldConf($option = null)
    {
        ...
        $filedvalid = MasterModel::inIt('field f')->field('f.field,f.type,f.extra,f.name,f.title,m.name as model_name')->where(array('m.name' => array('in', implode(',', $table))))->join("{$pix}model m", 'm.id=f.model_id', 'left')->select();
