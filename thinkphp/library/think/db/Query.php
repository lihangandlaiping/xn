...
class Query
{
     ...
     */
    public function insert(array $data, $replace = false, $getLastInsID = false, $sequence = null,$options=array())
    {
        // 分析查询表达式
        $options = $options?:$this->parseExpress();
        // 生成SQL语句
     ...
     * @return integer|string
     */
    public function insertAll(array $dataSet,$options=array())
    {
        // 分析查询表达式
        $options = $options?:$this->parseExpress();
    ...
    {
        $options =$options?$options: $this->parseExpress();
     ...
     */
    public function select($data = null,$options=array())
    {
     ...
     * @throws DataNotFoundException
     */
    public function find($data = null,$options=array())
    {
        if ($data instanceof Query) {
            return $data->find();
        } elseif ($data instanceof \Closure) {
     ...
     * @return array
     */
     function parseExpress($is_clear=true)
    {
