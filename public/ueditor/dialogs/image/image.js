...
(function () {
    ...
    function insertSearch(id, catchRemote) {
        ...
        for (var i = 0, ci; ci = imgs[i++];) {
            if (ci.getAttribute("selected")) {
                var url = ci.getAttribute("src", 2).replace(/(\s*$)/g, ""),data=ci.getAttribute("data", 2).replace(/(\s*$)/g, ""),img = {};
                img.src = url;
                img._src = url;
                img.data = data;
                imgObjs.push(img);
    ...
    /**
     * 插入单张图片
     * modify by liuwei at 2013-10-17
     * 以前返回一个数据{}，现在改为返回[]
     */
    function insertSingle() {
        ...
        var url = g("url"),
            ...
            imgObj = {},
            imgObjs = [];  //modify liuwei 2013-10-17 
        if (!url.value) return;
        ...
        imgObjs.data=url.data?url.data:"";
        imgObjs.push(imgObj);
        //insertImage(imgObj);
        insertImage(imgObjs);
    ...
    function insertBatch() {
        ...
        for (var i = 0, ci; ci = imageUrls[i++];) {
            ...
            tmpObj.data=editor.options.filePathDir + ci.url;
            imgObjs.push(tmpObj);
        }
        insertImage(imgObjs);
        hideFlash();
    ...
    function showImageInfo(img) {
        ...
        g("url").data=wordImgFlag ? wordImgFlag.replace("&amp;", "&") : (img.getAttribute('_src') || img.getAttribute("data", 2).replace("&amp;", "&"));
                        ...
                        ajax.request(editor.options.imageManagerUrl, {
                            ...
                            onsuccess:function (xhr) {
                                ...
                                for (var k = 0, ci; ci = imageUrls[k++];) {
                                    ...
                                    img.setAttribute('data',editor.options.filePathDir + ci.replace(/\s+|\s+/ig, ""));
                                }
                            },
                            onerror:function () {
